...
const AboutRestaurant = ({ route }: any) => {
    ...
    useEffect(() => {
        setLoading(true);
        const getData = async () => {
            const data = await axios.post('http://41.63.177.113:80/food/getRestaurantFood', {
        ...
        const fatchData = async () => {
            axios.post('http://41.63.177.113:80/addCart/getAllCount') .then(function(response) {
        ...
        const historyData = async () => {
            const historyData = await axios.post('http://41.63.177.113:80/history/getByType', {type: 'Food'});
                ...
                {historyData.length !== 0 ? <><View style={{flexDirection: 'row', justifyContent: 'space-between', width: width * 0.9, alignItems: 'center', marginBottom: 5}}>
                    ...
                    renderItem={({ item, index }: {item: any, index: number}) => (
                        <Card key={index} style={{ width: width * 0.28, marginBottom: 10, marginLeft: 5, marginRight: 5, backgroundColor:'white' }} onStartShouldSetResponder={(event) => true}>
                            <View onTouchEnd={() => navigation.navigate("Detail", {foodName: item.Name})}><Image source={{ uri: "http://41.63.177.113:80/uploads/Foodimage" + index % 5 + ".jpg" }} style={{width: '100%', height: width / 3, alignItems: 'flex-start', borderRadius: 10}} resizeMode='cover' /></View>
                ...
                <FlatList
                    ...
                    renderItem={({ item, index }: {item: any, index: number}) => (
                        <Card style={{ width: width * 0.28, marginBottom: 10, marginLeft: 5, marginRight: 5, backgroundColor: 'white' }} onTouchEnd={() => navigation.navigate("Detail", {foodName: item.Name})}>
                            <ImageBackground source={{ uri: "http://41.63.177.113:80/uploads/Foodimage" + index % 5 + ".jpg" }} style={{width: '100%', height: width / 3, alignItems: 'flex-start'}} imageStyle={{borderRadius: 10}}>
