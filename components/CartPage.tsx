...
const CartPage = () => {
    ...
    useEffect(() => {
        const getData = async () => {
            const data = await axios.post('http://41.63.177.113:80/addCart/getAllCart')
    ...
    const changeCount = async (value:number, productName:string) => {
        const data = await axios.post('http://41.63.177.113:80/addCart/changeProductCountByIndex', {productName: productName, value: value});
    ...
    const onDeleteAll = async () => {
        const data = await axios.post('http://41.63.177.113:80/addCart/emptyCart');
        console.log(data.data.success);
        if(data.data.success === true) {
            onRefresh();
        }
