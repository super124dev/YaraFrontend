...
const HomePage = () => {
    ...
    useEffect(() => {
        console.log("=========================")
        const getAll = async () => {
            const data = await axios.post('http://41.63.177.113:80/restaurant/getTypeRestaurant', {type: "Popular"})
            console.log("========================>", data.data);
            if (data.data.success === true) {
                setRestaurantPopularList(data.data.msg);
            }
            const data1 = await axios.post('http://41.63.177.113:80/restaurant/getTypeRestaurant', {type: "NearBy"})
            if (data.data.success === true) {
                setRestaurantNearByList(data1.data.msg);
            }
            const data2 = await axios.post('http://41.63.177.113:80/restaurant/getTypeRestaurant', {type: "Healthy"})
            if (data.data.success === true) {
                setRestaurantHealthyList(data2.data.msg);
            }
            const data3 = await axios.post('http://41.63.177.113:80/grocery/getTypeGrocery', {type: "Popular"})
            if (data3.data.success === true) {
                setGroceryPopularList(data3.data.msg);
            }
            const data4 = await axios.post('http://41.63.177.113:80/grocery/getTypeGrocery', {type: "NearBy"})
            if (data4.data.success === true) {
                setGroceryNearByList(data4.data.msg);
            }
            const data5 = await axios.post('http://41.63.177.113:80/grocery/getTypeGrocery', {type: "Healthy"})
            if (data5.data.success === true) {
                setGroceryHealthyList(data5.data.msg);
            }
            const data6 = await axios.post('http://41.63.177.113:80/pharma/getTypePharma', {type: "Popular"})
            if (data6.data.success === true) {
                setPharmaPopularList(data6.data.msg);
            }
            const data7 = await axios.post('http://41.63.177.113:80/pharma/getTypePharma', {type: "NearBy"})
            if (data7.data.success === true) {
                setPharmaNearByList(data7.data.msg);
            }
            const data8 = await axios.post('http://41.63.177.113:80/pharma/getTypePharma', {type: "Healthy"})
        ...
        <View style={styles.container}>
            <View style={{flexDirection: 'row', justifyContent: 'space-between', marginBottom: 5, width: '100%', alignItems: 'center'}}>
                <TouchableOpacity onPress={() => navigation.navigate("Tracking")}>
                    <Icon1 name="bike-fast" size={20} color="black" />
                </TouchableOpacity>
                <SelectDropdown
                    data={countries}
