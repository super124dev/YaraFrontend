...
const ForRestaurant = ({ route }: any) => {
    ...
    const [image, setImage] = useState("http://41.63.177.113:80/uploads/imagePicker.png");
    ...
    useEffect(() => {
        const getInfo = async () => {
            let data = await axios.post('http://41.63.177.113:80/order/getAll');
    ...
    const clickDelete = async () => {
        axios.post('http://41.63.177.113:80/order/deleteOne', {
            email: currentNumber
    ...
    const clickAddToRestaurant = async () => {
        ...
        await axios.post('http://41.63.177.113:80/restaurant/getRestaurantName', {
            ...
            if(response.data.success === true) {
                ...
                axios.post('http://41.63.177.113:80/food/createOne', {
                    Name: foodName,
                    ...
                    if(response.data.success === true) {
                        ...
                        setFoodPrice("");
                        setRestaurantName("");
                        setImage("http://41.63.177.113:80/uploads/imagePicker.png")
                    } else {
                        setChoosePrepared(true);
                    }
                    setLoading(false);
                ...
                <View style={styles.centeredView}>
                    <View style={styles.ImagemodalView}>
                        ...
                        <View style={{ width: 110, height: 110, borderRadius: 20, alignItems: 'center', borderWidth: 1, borderColor: 'gray', justifyContent: 'center' }}>
                            <TouchableOpacity onPress={() => chooseImage()}><Image source={{ uri: "http://41.63.177.113:80/uploads/gallery.png" }} style={{ width: 100, height: 100, borderRadius: 20 }} /></TouchableOpacity>
                        </View>
                        <View style={{ width: 110, height: 110, borderRadius: 20, alignItems: 'center', borderWidth: 1, borderColor: 'gray', justifyContent: 'center' }}>
                            <TouchableOpacity onPress={() => chooseCamera()}><Image source={{ uri: "http://41.63.177.113:80/uploads/camera.png" }} style={{ width: 100, height: 100, borderRadius: 20 }} /></TouchableOpacity>
                        </View>
                    </View>
                </View>
            </Modal>
